...
int
GetSkyMapSensitivity(double*& x, double*& y, double*& z, double*& t, TString& title, TString& ofileName, int& ndet) {
   ...
   for(int n=0;n<NDETECTORS;n++) D[n] = new detector((char*)detector_name[n].Data());
   ...
   for (int i=0;i<NTRIES;i++) {
      ...
      for (int k=0;k<NDETECTORS;k++) {
        if (detector_selected[k]) {
          sF[k] = D[k]->antenna(theta,phi,psi);
          F[k] = D[k]->antenna(theta,phi,0);
#ifdef USE_NOISE
//          X[k] = gRandom->Uniform(-1,1); // non GW signals
double A=1.;
//if (k==3) A=1.;
          X[k] = A*gRandom->Gaus(0,1); // non GW signals
#else
          X[k] = Hp*sF[k].real()+Hx*sF[k].imag();
#endif
          E+=X[k]*X[k];
#ifdef WRONG_DIRECTION
          F[k] = D[k]->antenna(gRandom->Uniform(0,180),gRandom->Uniform(0,360),0);
#endif
        }
      } 
      E=sqrt(E);
      for (int k=0;k<NDETECTORS;k++) {
        if (detector_selected[k]) {
          X[k] = X[k]*SNR/E + gRandom->Gaus(0.,1.);
        }
      } 
      ...
      double co = (gc+gR);                     // cos to DPF
      double si = gI;                          // sin to DPF
      double Fp_dpf[NDETECTORS];
      double Fx_dpf[NDETECTORS];
      double mFp_dpf = 0;
      double mFx_dpf = 0;
      for (int k=0;k<NDETECTORS;k++) {
        if (detector_selected[k]) {
          Fp_dpf[k]=F[k].real()*co+F[k].imag()*si;
          mFp_dpf+=Fp_dpf[k]*Fp_dpf[k];
          Fx_dpf[k]=-F[k].real()*si+F[k].imag()*co;
          mFx_dpf+=Fx_dpf[k]*Fx_dpf[k];
        } else {
          Fp_dpf[k]=0;
          Fx_dpf[k]=0;
        }
      ...
      double ri=rip*rip/mFp_dpf+rix*rix/mFx_dpf; 
      ...
      double Xri=Xrip*Xrip/um+Xrix*Xrix/vm; 
...
// Compute Network Detector Index
      ...
      //double ndi=net9(Fp_dpf,mFp_dpf,Fx_dpf,mFx_dpf,GAMMA);
      double uFp=0; 
      double uFx=0; 
      for (int k=0;k<NDETECTORS;k++) {
        if (detector_selected[k]) {
      ...
      uFp/=sqrt(mFp_dpf*um);
      uFx/=sqrt(mFx_dpf*um);
      double angle = fabs(atan2(uFx,uFp)*180./pi);
      if (angle>90) angle=180.-angle;

      double wm=0;                  // weak cos
      double om=0;                  // hard cos
      double cw=0;                  // weak cos
      double ch=0;                  // hard cos
      double Et=0;                  // energy
      for (int k=0;k<NDETECTORS;k++) {
        if (detector_selected[k]) {
          cw+=u[k]*X[k];
          ...
          Et+=X[k]*X[k];
          wm+=u[k]*u[k];
          om+=Fp_dpf[k]*Fp_dpf[k];
        }
      }
      double sh = (1-cw*cw/(wm*Et))/(1-ch*ch/(om*Et)); // sin^2 between the planes
      if (sh<0) sh=0;
      ...
      double Fp = sqrt(gr+gc);
      double Fx = sqrt(gr-gc);
      t[cnt]=Fx;
