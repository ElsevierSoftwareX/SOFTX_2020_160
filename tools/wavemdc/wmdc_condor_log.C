{
  ...
  TString logFile = TString(ifoLabel)+"-"+frLabel+"-Log.txt";

  bool first=true;

  vector<TString> logList;

  vector<TString> dirList = TB.getFileListFromDir(frDir, "", "", frLabel);
  ...
  if (!out.good()) {cout << "LogWaveMDC.C - Error Opening File : " << lstFile.Data() << endl;exit(1);}
  ...
  if (!in.good()) {cout << "LogWaveMDC.C - Error Opening Sorted File : " << sortFile.Data() << endl;exit(1);}
  ...
  MDC.SetSkyDistribution(MDC_LOGFILE,frDir+"/"+logFile,par);
  MDC.DumpLog(frDir+"/"+rootFile,frLabel);

  // write log header
  TString logHeaderFile = logFile;
  logHeaderFile.ReplaceAll("-Log.txt","-LogHeader.txt");
