...

{
   ...
   if(nIFO>3) {
      cout << "cwb_report_slag.C : Skipping plot production - wave tree should come from a 2- or 3-fold network : " 
           << net_file_name << endl;
      exit(1);
   } 
   ...
   mynext.Reset();
   if(!slagFound) {
      cout << "cwb_report_slags.C : Error - wave tree does not contain slag leaf : " 
           << net_file_name << endl;
      exit(1);
   }
   ...
   for(int i =1 ;i<NSlag+1;i++){
      for(int j =1 ;j<NSlag+1;j++){
	    ...
	    mycount++;
	    if(i*j%100==0) {
               cout << "slag[1] :"<<xaxis->GetBinCenter(i)<<" slag[2] :"<<yaxis->GetBinCenter(j) 
                    <<" FAR :"<<FAR[mycount-1] << "+/-" <<eFAR[mycount-1] << " " <<endl;}
 	 }
      }		
   }
   ...
   for(int i =0 ;i<mycount;i++) eN[i]=0.0;
   ...
   FarPlot->GetHistogram()->SetXTitle("time shift [day]");
   FarPlot->GetHistogram()->SetYTitle("FAR [1/year]");
   FarPlot->GetHistogram()->GetXaxis()->SetTitleSize(0.05);
   FarPlot->GetHistogram()->GetYaxis()->SetTitleSize(0.05);
   FarPlot->GetHistogram()->GetXaxis()->SetLabelSize(0.05);
   FarPlot->GetHistogram()->GetYaxis()->SetLabelSize(0.05);
   c2->Clear();
   c2->SetLogy(kFALSE);
