...
typedef struct {                         // structure for output for estimated parameters
...
int ChirpMass(TString ifroot, int gtype=0, int entry=0, int ifo=0, EBBH* xebbh=NULL) {
  ...
  //
  // entry  : entry id of the event in the root file
  //
  // ifo    : detector id : 0,1,... (see definition in user_parameters.C)
  //
  // xebbh  : pointer to the EBBH structure : used by a macro to return the ebbh parameters
  // 
  // Ex     : root 'ChirpMass.C("root_file.root",1,0,0)'
  //
...
*/
  ...
  if(gTYPE==8) {
  	                   MDC->SetZoom(0.999);
                           watplot* plot=NULL;
                           if(ebbh.winj[ifo].size()!=0) {
                             plot=MDC->Draw(ebbh.winj[ifo],MDC_TIME,"ALP ZOOM",kRed);
                             MDC->Draw(ebbh.wrec[ifo],MDC_TIME,"SAME",kBlack);
                           } else {
                             plot=MDC->Draw(ebbh.wrec[ifo],MDC_TIME,"ALP ZOOM",kRed);
                           }
  ...
  if(gTYPE==9) {
  	                   MDC->SetZoom(0.999);
                           watplot* plot=NULL;
                           if(ebbh.winj[ifo].size()!=0) {
                             plot=MDC->Draw(ebbh.winj[ifo],MDC_FFT,"ALP ZOOM",kRed);
                             MDC->Draw(ebbh.wrec[ifo],MDC_FFT,"SAME",kBlack);
                           } else {
                             plot=MDC->Draw(ebbh.wrec[ifo],MDC_FFT,"ALP ZOOM",kRed);
                           } 
                           if(plot) plot->graph[0]->SetTitle(TString::Format("%s : Rec(black), Inj(red))",ebbh.name.Data()));
  ...
  if(gTYPE==13)            {DrawMChirpInstantaneousFrequency(&ebbh, "winj", ifo); return 0;}
  if(gTYPE==14)            {DrawMChirpInstantaneousFrequency(&ebbh, "wrec", ifo); return 0;}
  if(gTYPE==15)            {DrawMChirpInstantaneousFrequency(&ebbh, "wsim", ifo); return 0;}
...
// ----------------------------------------------------
  ...
  for(int n=0;n<nIFO;n++) {
    ...
    ebbh->winj[n] = wINJ[n]->size() ? GetAlignedWaveform(wINJ[n],wREC[n]) : 0;
...
void PlotChirp(EBBH* ebbh, int mch_order, double& tmerger, double& xmin, double& xmax, EColor color) {
  ...
  double echirp = ebbh->nc.mchirp(1,chi2_thr,tmerger_cut,zmax_thr);
...
void PlotMChirp(EBBH* ebbh) {
  ...
  for(int i=0; i<NCHIRP_MAX; i++) {
    ...
    ebbh->nc.mchirp(1,chi2_thr,tmerger_cut,zmax_thr);
...
void DrawMChirpInstantaneousFrequency(EBBH* ebbh, TString wtype, int ifo) {
  ...
  cout << "mchirp - mchirp  : " << imchirp << endl;
  cout << "mchirp - tmerger : " << tmerger << endl;
