...
// Extract whitened reconstructed waveforms, and compute the Qveto & Lveto parameters
  ...
  cout << endl;
  cout << "-----> CWB_Plugin_Qveto.C" << endl;
  cout << "ifo " << ifo.Data() << endl;
  cout << "type " << type << endl;
  cout << endl;

  float Qveto[NIFO_MAX];				// Qveto
  float Lveto[3];                               	// Lveto
  ...
  if(type==CWB_PLUGIN_ILIKELIHOOD) {
    ...
    if (files) {                                   
      ...
      if(!froot) {                                                    
        cout << "CWB_Plugin_Qveto.C : Error - output root file not found" << endl;
        gSystem->Exit(1);                                                                             
      }                                                                                      
    } else {                                                                                 
      cout << "CWB_Plugin_Qveto.C : Error - output root file not found" << endl;  
    ...
    if(net_tree==NULL) {
      ...
      net_tree->Branch("Qveto",Qveto,TString::Format("Qveto[%i]/F",cfg->nIFO));
    ...
    for(int k=0; k<K; k++) {  				// loop over the lags
      ...
      for(int j=0; j<(int)id.size(); j++) {  		// loop over cluster index
        ...
        for(int n=0; n<nIFO; n++) {
           ...
           cout << "Qveto : " << pd->Name << " Qveto = " << Qveto[n] << endl;
        }
        cout << "----------------------------------------------------------------" << endl;
        delete [] pwfREC;
        ...
        if(cfg->dump) {                                 
          ...
          for(int i=0; i<nIFO; i++) fprintf(EVT->fP,"%f ",Qveto[i]);
          fprintf(EVT->fP,"\n");
...
//   
  ...
  for(int n=0; n<V; n++) {
    ...
    if(pix->layers%2==0) {        
      cout << "CWB_Plugin_Qveto.C - Error : is enabled only for WDM (2G)" << endl;
      exit(1);                                                                                     
    }                                                                                              
