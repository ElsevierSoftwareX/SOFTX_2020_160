...
void Posterior2XML(TString gwname, int seed=-1, TString model="");
...
void GWOSC_Tools(TString tool, TString spar1="", TString spar2="", double dpar1=0) {
  ...
  if(tool=="Posterior2XML") 	Posterior2XML(spar1,int(dpar1),spar2);
  if(tool=="ConvertPSD") 	ConvertPSD(spar1);
  if(tool=="ConvertPSD2ASD") {	
    vector<TString> fileList = CWB::Toolbox::getFileListFromDir(".",".dat",spar1,"",true);
    for(int i=0;i<fileList.size();i++) {
      TString ifnamePSD = fileList[i];
      TString ofnameASD = fileList[i]; ofnameASD.ReplaceAll("PSD","ASD");
      ConvertPSD2ASD(ifnamePSD,ofnameASD);
    }
  }
}
...
void Posterior2XML(TString gwname, int seed, TString model) {
//
...
// how to download, eg: wget https://dcc.ligo.org/public/0157/P1800370/005/GW150914_GWTC-1.hdf5 -q -O GW150914.hdf5
//
// input:	gwname: GW name
//		seed:	seed used to randomly select a sample from the gwname+"_PE.dat" posterior sample file
//			if =0 then the maxl sample is selected
//			if =1 then the map sample is selected
//			if <0 then the first sample is selected
//
//              model:  PE approximant    
//
  ...
  if(seed==0)        options += "--sample maxl ";
  else if(seed==1)   options += "--sample map ";
  else               options += TString::Format("--seed %d ",seed);

  TString file_tag = model!="" ? gwname+"_PE_"+model : gwname+"_PE";
  TString approximant = model; approximant.ReplaceAll("C01:","");

  if(model=="") approximant = "IMRPhenomPv2";	// used for GWTC-1 catalog

  options += "--source "+gwname+" ";
  options += "--waveform "+approximant+"pseudoFourPN ";
  options += "--f_ref 20 ";
  options += "--f_lower 20 ";

  TString posteriorFile  = file_tag+".dat";

  TString xmlFile        = file_tag+".xml";
  options += "--clb_file "+file_tag+".clb ";
...
void ConvertPSD2ASD(TString ifnamePSD, TString ofnameASD) {
  ...
  ifstream in;
  in.open(ifnamePSD.Data(),ios::in);
  if (!in.good())
    {cout << "ConvertPSD2ASD - Error Opening File : "
          << ifnamePSD.Data() << endl;gSystem->Exit(1);}

  cout << "ConvertPSD2ASD - Read File : " << ifnamePSD.Data() << endl;

  vector<double> vfr;
  vector<double> vsh;
  char sfr[1024],ssh[1024];
  char str[1024];
  double fr,sh;
  ...
  while (1) {
    ...
    sh = TString(ssh).Atof(); 
    if(sh<=0)
      {cout << "ConvertPSD2ASD - input sensitivity file : " << ifnamePSD.Data()
            << " contains zero at frequency : " << fr << " Hz " << endl;gSystem->Exit(1);}
    vfr.push_back(fr); vsh.push_back(sh);
  }
  in.close();

  // set high sh values at beginning and end frequency range to exclude that frequency from the analysis
  vsh[0] = vsh[1];
  fr = 2*vfr[vfr.size()-1] - vfr[vfr.size()-2];
  sh = vsh[vsh.size()-1];
  vfr.push_back(fr); vsh.push_back(sh);
  vfr.push_back(8192.); vsh.push_back(sh);

  for(int i=0;i<vsh.size();i++) if(vsh[i]>SH_SUP) vsh[i]=SH_SUP;

  // overwrite input psd with the PSD values ASD=sqrt(PSD)
  cout << "ConvertPSD2ASD - Write File : " << ofnameASD.Data() << endl;
  ofstream out;
  out.open(ofnameASD.Data(),ios::out);
  for(int i=0;i<vfr.size();i++) out << vfr[i] << "\t" << sqrt(vsh[i]) << endl;	// oneside spectrum
  out.close();
