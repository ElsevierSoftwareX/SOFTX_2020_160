...
//  fLow  = 64.;        // kill V1 noise - Used with Tukey Window
...
//  subnet = 0.0;		// Used with Tukey Window
  ...
  Acore  = 1.7;
  Tgap   = 0.2;
//  Tgap   = 0.8;		// Used with Tukey Window
  Fgap   = 128.0;
  delta  = 0.5;
  cfg_gamma  = -1.0;	
  LOUD   = 300;
...
//  jobfOptions = CWB_JOBF_SAVE_CSTRAIN;  // save cstrain data into the final ROOT file
...
//  strcpy(channelNamesRaw[0],"L1:GWOSC-4KHZ_R1_STRAIN");       // Used with Tukey Window
  ...
  strcpy(channelNamesRaw[2],"V1:GWOSC-4KHZ_R1_STRAIN");
...
//  #define NDFQ 12	// Used with Tukey Window
  #define NDFQ 11   
  nDQF=NDFQ;
  dqfile dqf[NDFQ]={
...
*/
  ...
  optwf += "sn_strain_file_name=input/V1_ASD.txt ";    // add V1 strain file name to second detector
  ...
  optwf += "sn_seed=3 ";                    // add seed for random noise generation to V1 detector
