...
CWB_Plugin(TFile* jfile, CWB::config* cfg, network* net, WSeries<double>* x, TString ifo, int type)  {
  ...
  if(type==CWB_PLUGIN_DATA) {  
    ...
    int seed;
    if(ifo.CompareTo("L1")==0) seed=10000;
    if(ifo.CompareTo("H1")==0) seed=20000;
    if(ifo.CompareTo("V1")==0) seed=30000;
    if(ifo.CompareTo("J1")==0) seed=40000;
    if(ifo.CompareTo("A2")==0) seed=50000;
    if(ifo.CompareTo("Y2")==0) seed=60000;
    if(ifo.CompareTo("Y3")==0) seed=70000;

    TString fName;
    if(ifo.CompareTo("L1")==0) fName="detectors_PSD/LIGO_zero_det_HP.txt";
    if(ifo.CompareTo("H1")==0) fName="detectors_PSD/LIGO_zero_det_HP.txt";
    if(ifo.CompareTo("V1")==0) fName="detectors_PSD/VIRGO.txt";
    if(ifo.CompareTo("J1")==0) fName="detectors_PSD/KAGRA.txt";
