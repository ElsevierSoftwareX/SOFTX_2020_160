...
void CreateFramesNOISE() {
  ...
  TString IFOS[5] = {"L1",  /* LIGO Livingston                   */
                     "H1",  /* LIGO Hanford                      */
                     "H2",  /* LIGO Hanford                      */
                     "H3",  /* LIGO Hanford                      */
                     "V1"   /* Virgo                             */
  ...
  int  nIFO = 5;           // size of network starting with first detector ifo[]
...
GetNoise(wavearray<double>& x, TString ifo, int run) {
  ...
  if(ifo.CompareTo("H2")==0) seed=3000;
  if(ifo.CompareTo("H3")==0) seed=4000;
  if(ifo.CompareTo("V1")==0) seed=5000;
  if(ifo.CompareTo("J1")==0) seed=6000;
  if(ifo.CompareTo("I1")==0) seed=7000;
