...
void CreateFramesNOISE() {
  ...
  TString IFOS[4] = {"L1",  /* LIGO Livingston                   */
                     "Y2",  /* LIGO Hanford                      */
                     "Y3",  /* LIGO Hanford                      */
                     "V1"   /* Virgo                             */
                    }; 

  int  nIFO = 4;           // size of network starting with first detector ifo[]
...
GetNoise(wavearray<double>& x, TString ifo, int run) {
  ...
  if(ifo.CompareTo("Y2")==0) seed=4000;
  if(ifo.CompareTo("Y3")==0) seed=5000;
