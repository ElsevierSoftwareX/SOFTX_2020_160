...
//estraggo le info che servono anche al tree di def dell'mlp
   ...
   char NOMEtot[1024];
   cout<<deltaMc<<endl;
   //sprintf(NOMEtot,"NN_%s_FILE_%s_TS%i_TB%i_st%i_bt%i_dMc%1.2f_uf%i",NNi0.Data(),Filei0.Data(),TS,TB,s,b,deltaMc,uf);
   sprintf(NOMEtot,"NN_%s_FILE_%s_TS%i_TB%i_st%i_bt%i_uf%i_dMc%i",NNi0.Data(),Filei0.Data(),TS,TB,s,b,uf,deltaMc);
...
char ofile[1024];
sprintf(ofile,"outfile_Mc/%s.root",NOMEtot);
...
//   ent0=
   ...
   for(int n=s;n<s+TS;n++) {
    ...
    cout<<"rho: "<<signal.rho[0]<<" cc "<<signal.netcc[1]<<" Mc: "<<Mc<<endl; 
     NNTree2->Fill();
    }
int entSig1=0;
entSig1=NNTree2->GetEntries();      
cout<<entSig1<<endl;
...
int bg_05=0;
   ...
   for(int n=b;n<b+TB;n++) {
    ...
    cout<<"rho: "<<background.rho[0]<<" cc "<<background.netcc[1]<<" Mc: "<<Mc<<endl; 
     NNTree2->Fill();
     }
...
cout<<" Realentries "<<Realentries<<" entSig: "<<entSig1<<endl;

}

void graph(TString ifile){
	TString name(ifile);
	name.ReplaceAll("outfile_Mc/","");
...
//	cout<<"dopo Draw()"<<endl;
	...
	char CnameB2root[1024];
        cout<<"e ora non più"<<endl;
	CnameS.ReplaceAll(".root",".png");
	CnameB.ReplaceAll(".root",".png");
        cout<<CnameS.Data()<<endl;
        //sprintf(CnameS2,"logMc_rho/logN_rho_S_dMc%1.2f",deltaMc);
        sprintf(CnameS2,"logMc_rho/logN_rho_S_dMc%i",deltaMc);
        //cout<<"e ora non più"<<endl;
        cout<<CnameS2<<endl; 
	//sprintf(CnameS2,"logMc_rho/logN_rho_S_dMc%1.2f_%s",deltaMc,CnameS.Data());
	sprintf(CnameS2,"logMc_rho/logN_rho_S_dMc%i_%s",deltaMc,CnameS.Data());
        cout<<CnameS2<<endl;
	//sprintf(CnameB2,"logMc_rho/logN_rho_B_dMc%1.2f_%s",deltaMc,CnameB.Data());
	sprintf(CnameB2,"logMc_rho/logN_rho_B_dMc%i_%s",deltaMc,CnameB.Data());
        cout<<CnameB2<<endl;
	//sprintf(CnameS2root,"logMc_rho/logN_rho_S_dMc%1.2f_%s",deltaMc,CnameSroot.Data());
	sprintf(CnameS2root,"logMc_rho/logN_rho_S_dMc%i_%s",deltaMc,CnameSroot.Data());
        cout<<CnameS2root<<endl;
	//sprintf(CnameB2root,"logMc_rho/logN_rho_B_dMc%1.2f_%s",deltaMc,CnameBroot.Data());
	sprintf(CnameB2root,"logMc_rho/logN_rho_B_dMc%i_%s",deltaMc,CnameBroot.Data());
        cout<<CnameB2root<<endl;
        cout<<"e ora non più"<<endl;
	cS->SaveAs(CnameS2);
	cB->SaveAs(CnameB2);
...
//      int nBg=0;
  ...
  TCanvas* cS=new TCanvas("Efficiency_vs_ANN","Efficiency_vs_ANN",0,0,1200,700);
	...
	cS->cd(3);
        ...
        cS->cd(4);
        TMultiGraph* mg4=new TMultiGraph();
        mg4->SetTitle("cc=0.65;Mc;#Events");
        for (int h=0;h<nRHO;h++){
	...
	TCanvas* cB=new TCanvas("Number_vs_Mc","Number_vs_Mc",0,0,1200,700);
        cB->Divide(2,2);
        cB->cd(1)->SetLogy();
        TMultiGraph* mg1B=new TMultiGraph();
        mg1B->SetTitle("cc=0.5;Mc;#Events");
        for (int h=0;h<nRHO;h++){
                gB[h]->SetLineColor(4);
                if(gB[h]->GetN()!=0) mg1B->Add(gB[h]);
                }
        ...
        cB->cd(2)->SetLogy();
        TMultiGraph* mg2B=new TMultiGraph();
        mg2B->SetTitle("cc=0.55;Mc;#Events");
        for (int h=0;h<nRHO;h++){
        ...
        cB->cd(3)->SetLogy();
        TMultiGraph* mg3B=new TMultiGraph();
        mg3B->SetTitle("cc=0.6;Mc;#Events");
        for (int h=0;h<nRHO;h++){
        ...
        cB->cd(4)->SetLogy();
        TMultiGraph* mg4B=new TMultiGraph();
        mg4B->SetTitle("cc=0.6;Mc;#Events");
        for (int h=0;h<nRHO;h++){
...
//	dANNth=1./nth;
	...
	for (int d=0; d<nth+1;d++) bins[d+1]=0.+(d+1)*deltaMc;
...
//	TH2D* hglitch=new TH2D("Survived_glitches(RHO_cut:5.5)","Survived_glitches(RHO_cut:5.5)",nth,0.5,1,nth+1,0.,1.);
	...
	for (int b=0;b<nth;b++) for(int c=0;c<nth+1;c++) {
			...
			hglitch->Fill(cc1th[b]+dccth/2.,bins[c]+deltaMc/2.,Z[(nth+1)*b+c]);
	...
	c->cd(2);
        TMultiGraph* mg2=new TMultiGraph();
	mg2->SetTitle("cc_Mcoutput;cc;Mcoutput");
	if(gB[1]->GetN()!=0) mg2->Add(gB[1]);
	if(gS[1]->GetN()!=0) mg2->Add(gS[1]);
	mg2->Draw("ap");
	c->cd(3);
        TMultiGraph* mg3=new TMultiGraph();
	mg3->SetTitle("rho_Mcoutput;rho;Mcoutput");
	if(gB[2]->GetN()!=0) mg3->Add(gB[2]);
	if(gS[2]->GetN()!=0) mg3->Add(gS[2]);
	mg3->Draw("ap");
	c->cd(4);
	TText* text=new TText(0.37,0.0,"no cuts on Mc");
	hglitch->SetStats(0);
	hglitch->GetXaxis()->SetTitle("cc");
	hglitch->GetYaxis()->SetTitle("Mcoutput");
   ...
   Cnameroot.ReplaceAll("outfile_Mc/","");
   Cname.ReplaceAll("outfile_Mc/","");
   Cname.ReplaceAll(".root",".png");
   sprintf(Cname2,"CC_RHO_Mc_Plots/CC_RHO_Mc_Plots_%s",Cname.Data());
   sprintf(Cname2root,"CC_RHO_Mc_Plots/CC_RHO_Mc_Plots_%s",Cnameroot.Data());
   c->SaveAs(Cname2);
   c->Print(Cname2root);
	...
	gB[2]->Draw("p,same");
	c2->cd(4);
	TText* text2=new TText(0.37,0.0,"no cuts on Mc");
	hglitch->SetStats(0);
	hglitch->GetXaxis()->SetTitle("cc");
	hglitch->GetYaxis()->SetTitle("Mcoutput");
	hglitch->GetZaxis()->SetTitle("count");
  ...
  Cname_2.ReplaceAll("outfile_Mc/","");
  Cname_2root.ReplaceAll("outfile_Mc/","");
  Cname_2.ReplaceAll(".root",".png");
   sprintf(Cname2_2,"CC_RHO_Mc_Plots/CC_RHO_Mc_Plots_BoS_%s",Cname_2.Data());
   sprintf(Cname2_2root,"CC_RHO_Mc_Plots/CC_RHO_Mc_Plots_BoS_%s",Cname_2root.Data());
   c2->SaveAs(Cname2_2);
