...
//					cout<<" dentro if"<<endl;
   ...
   char NNibis[1024];
   char NNi2bis[1024];
   sprintf(NNi2bis,"%s",NN_FILEbis.Data());
   while (NNi2bis[pbis]){
	//cout<<NNi2[p]<<endl;
	if (NNi2bis[pbis]=='N'){
//	cout<<p<<endl;
                        if((NNi2bis[pbis+1]=='N')&&(NNi2bis[pbis+2]=='\/')) {
//					cout<<" dentro if"<<endl;
					int hh=pbis+3;
					while (NNi2bis[hh]!='\0'){NNibis[hh-p-3]=NNi2bis[hh];hh=hh+1;}
					break;
					}
	}
	pbis=pbis+1;
...
// cout<<Filei2[q]<<endl;
 if(Filei2[q]=='n'&&Filei2[q+1]=='n'&&Filei2[q+2]=='T'&&Filei2[q+3]=='R'&&Filei2[q+4]=='E'&&(Filei2[q+5]=='E')&&(Filei2[q+6]=='\/')) {
   ...
   TFile* fnetbis =TFile::Open(NN_FILEbis.Data());
   TMultiLayerPerceptron *mlpbis = (TMultiLayerPerceptron*)fnetbis->Get("TMultiLayerPerceptron");
   if(mlpbis==NULL) {cout << "Error getting mlp" << endl;exit(1);}
   TTree* infotbis=(TTree*)fnetbis->Get("info");
   int NNsbis;
   int NNbbis;
   int NNnTSbis;
   int NNnTBbis;
   infotbis->SetBranchAddress("Rand_start_Sig",&NNsbis);
   infotbis->SetBranchAddress("Rand_start_Bg",&NNbbis);
   infotbis->SetBranchAddress("#trainSig",&NNnTSbis);
   infotbis->SetBranchAddress("#trainBg",&NNnTBbis);
   infotbis->GetEntry(0);
...
//estraggo le info che servono anche al tree di def dell'mlp
   ...
   char NOMEtot2[1024];
   sprintf(NOMEtot2,"N1_%s_N2_%s_F_%s_TS%i_TB%i_st%i_bt%i_uf%i",NNi0.Data(),NNi0bis.Data(),Filei0.Data(),TS,TB,s,b,uf);
   TString NOMEtot_S(NOMEtot2);
   NOMEtot_S.ReplaceAll("nnTree","");
   NOMEtot_S.ReplaceAll("s0","");
   NOMEtot_S.ReplaceAll("b70000","");
   NOMEtot_S.ReplaceAll("ROC","");
   NOMEtot_S.ReplaceAll("approx","ap");
   NOMEtot_S.ReplaceAll("mlpNetwork","");
   NOMEtot_S.ReplaceAll("structure","str");
   NOMEtot_S.ReplaceAll("epochs","ep");
   NOMEtot_S.ReplaceAll("colored","rec");
   NOMEtot_S.ReplaceAll("__","_");

   char NOMEtot[1024];
   //sprintf(NOMEtot,"NN1_%s_NN2_%s_FILE_%s_TS%i_TB%i_st%i_bt%i_uf%i",NNi0.Data(),NNi0bis.Data(),Filei0.Data(),TS,TB,s,b,uf);
   sprintf(NOMEtot,"%s",NOMEtot_S.Data());
   cout<<"nome: "<<NOMEtot<<endl;
   char Nometot2[1024]="comp_orig_dt_a_21marzo_50000";
   TString SIG_FILE;
   TString BG_FILE;
   char FILE_NAME[516];
...
char ofile[1024];
sprintf(ofile,"outfile_2NN/%s.root",Nometot2);
...
NNTree2->SetDirectory(f);
  ...
  NNTree2->Branch("ANN2out",&outbis,"ANN2out/D");
  NNTree2->Branch("cc",&NNcc,"cc/D");
  NNTree2->Branch("rho",&NNrho,"rho/D");
  char NNfilenamebis[1024];
  NNTree2->Branch("NN2name",&NNfilenamebis,"NN2name/C");
  sprintf(NNfilenamebis,"%s",NN_FILEbis.Data());
...
void Plots(TString ifile){
	...
	int nSi;
	NNTree2->SetBranchAddress("ANNout",&out);
	NNTree2->SetBranchAddress("ANN2out",&outbis);
	NNTree2->SetBranchAddress("cc",&cc);
	NNTree2->SetBranchAddress("rho",&rho);
	NNTree2->SetBranchAddress("#TestSig",&nSi);
	NNTree2->GetEntry(0);
...
//	cout<<"nSig: "<<nSig<<endl;
	for (int n = 0; n <NNTree2->GetEntries(); n++){
		 NNTree2->GetEntry(n);
		 if(n<nSig) {
			gS[0]->SetPoint(n,outbis,out);
	                cout<<"Sig_graph1: x="<<outbis<<" y: "<<out<<endl;
		}
		else {
			gB[0]->SetPoint(n-nSig,outbis,out);
	                cout<<"Bg_graph1: x="<<outbis<<" y: "<<out<<endl;
		}
	}
	...
	c->cd(1);
        TMultiGraph* mg1=new TMultiGraph();
	mg1->SetTitle("out1_out2");
	if(gB[0]->GetN()!=0) mg1->Add(gB[0]);
	if(gS[0]->GetN()!=0) mg1->Add(gS[0]);
	mg1->Draw("ap");
	c->cd(2);
        TMultiGraph* mg2=new TMultiGraph();
	mg2->SetTitle("out1_out2");
   ...
   sprintf(Cname2,"CC_RHO_ANN_Plots_2NN/out_Plots_%s",Cname.Data());
   sprintf(Cname2root,"CC_RHO_ANN_Plots_2NN/out_Plots_%s",Cnameroot.Data());
   c->SaveAs(Cname2);
   c->Print(Cname2root);
/*
