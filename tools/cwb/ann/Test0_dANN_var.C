...
//estraggo le info che servono anche al tree di def dell'mlp
   ...
   char NOMEtot[1024];
   sprintf(NOMEtot,"NN_%s_FILE_%s_TS%i_TB%i_st%i_bt%i_dANN%1.2f_uf%i",NNi0.Data(),Filei0.Data(),TS,TB,s,b,deltaANN,uf);
...
//	gS[0]->SetMarkerStyle(7);
	...
	if(gS[1]->GetN()!=0) mg1->Add(gS[1]);
	if(gS[0]->GetN()!=0) mg1->Add(gS[0]);
//	gS[0]->Draw("apl");
	for (int h=2;h<nANN;h++){
...
//	gS[nANN*2]->SetMarkerStyle(7);
	...
	if(gS[nANN*2+1]->GetN()!=0) mg3->Add(gS[nANN*2+1]);
