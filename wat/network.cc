...
long network::likelihoodWP(char mode, int lag, int iID, TH2F* hist)
{
...
// fill in backward delay configuration
	 ...
	 Nw += null;
      ...
      pwc->cData[id-1].netRHO  = rho/sqrt(cc);               // reguced rho - stored in rho[0]
...
long network::subNetCut(int lag, float subnet, float subcut, TH2F* hist)
{
...
// return number of processed pixels
   ...
   subcut = fabs(subcut);                    // sub network threshold in the skyloop
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  skyloop:
	 ...
	 for(j=0;j<vint->size();j++) hist->Fill(suball,submra);
      ...
      if(fmin(suball,submra)>subnet && rHo>this->netRHO) {
         count += vint->size();	
	 if(hist) {
