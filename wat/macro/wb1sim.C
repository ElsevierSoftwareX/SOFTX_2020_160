...
double bpp        = 0.005;  // black pixel probability
double sT         = 2.0;    // black pixel probability
...
char outFile[512];
sprintf(outFile,"wave_%d_%d%s.root",
	start,int(100.*bpp),suffix);
...
for(j=0; j<lags; j++){
   ...
   cout<<cX[j].merge(sT)<<" ";
   cout<<cY[j].merge(sT)<<" ";
   cout<<cZ[j].merge(sT)<<endl;

   cX[j].setrms(nX); cY[j].setrms(nY); cZ[j].setrms(nZ);
   cX[j].setvar(vx); cY[j].setvar(vy,0.,768.); cZ[j].setvar(vz);
...
cout<<"Exp done\n";
   ...
   for(j=0; j<lags; j++){                // L1 time shifts
	 ...
	 cout<<" stride="<<S0.stride()<<"  offset="<<m<<endl;
      ...
      printf(" L1H1: %6f",xcXY.coincidence(0.1,&xcYZ));
      printf(" H1H2: %6f",xcYZ.coincidence(0.1,&xcXY));
      printf(" H2L1: %6f",xcZX.coincidence(0.1,&xcYZ)); cout<<endl;
      cout<<"  coincidence occupancy: ";
      printf(" L1H1: %6f",xcXY.coincidence(0.1,&xcZX));
      printf(" H1H2: %6f",xcYZ.coincidence(0.1,&xcZX));
      printf(" H2L1: %6f",xcZX.coincidence(0.1,&xcXY)); cout<<endl;
