...
// 4 ifo, max delay 0.031 sec
//double pln[27]={2.700,0.254,0.200,   /* level 1 */
//		2.500,0.274,0.200,   /* level 2 */
//                2.350,0.270,0.161,   /* level 3 */
//		2.170,0.288,0.140,   /* level 4 */
//		1.970,0.306,0.130,   /* level 5 */
//		1.776,0.322,0.128,   /* level 6 */
//		1.600,0.334,0.128,   /* level 7 */
//		1.450,0.340,0.138};  /* level 8 */
...
WSeries<float>   vx,vy,vz,vg;   // noise variability
WSeries<double>  nX,nY,nZ,nG;   // noise rms
...
NET.add(&L1); NET.add(&H1); 
NET.add(&H2); NET.add(&G1);               // add detectors to network
...
An=G1.antenna(theta,phi,psi);
yp *= An.real();
yx *= An.imag();
delay = G1.tau.get(iTheta,iPhi);
ii = int(delay*yp.rate());
cout<<"H2 delay = "<<delay<<"  "<<ii<<" F+ = "<< An.real()<<" Fx = "<< An.imag()<<endl;

sprintf(file,"/home/klimenko/wat/wat-4.2.8/plots/geo_g1.lst");
readframes(file,"H2:LSC-STRAIN",x);
x.start(L1.getTFmap()->start());
//sprintf(file,"/home/klimenko/wat/wat-4.0.RH/plots/v1_b1.lst");
//readframes(file,"H2:LSC-STRAIN",x);
//readframes(file,"H2:noise",x);
//ii=0; readframes(file,"H2:DFM_A1B2G1",y); yp.cpf(y,16384,int(89.5*16384)); yp*=3;
...
ii = int(yp.rate()*G1.tau.get(G1.tau.indexTheta(Theta),G1.tau.indexPhi(Phi))); 
wB.rate(x.rate());
wB.start(x.start());
wB.resize(x.size());
if(ii>0) wB.cpf(x,x.size()-ii,0,ii);
...
G1.getTFmap()->setlow(768.);
G1.getTFmap()->lprFilter(1.,0,120.,4.);
G1.white(100.,w_mode,8.,20.);
vg=G1.getTFmap()->variability();
...
for(i=0; i<n; i++){
   ...
   G1.getTFmap()->getLayer(x,i); x = 0.; G1.getTFmap()->putLayer(x,i);  // zero level i
}
*/
...
//   L1.writeFilter("/home/klimenko/wat/wat-4.0.RH/data/Meyer1024_L7.dat");
    ...
    L1.readFilter(filtername); H1.setFilter(L1); H2.setFilter(L1);  G1.setFilter(L1); 
    NET.setFilter(&H1,0.031); NET.setDelay(&H1); NET.setDelay(&H2); NET.setDelay(&G1);
    
    Ao=logNormArg(bpp,pLN[0],pLN[1],pLN[2]);
    cout<<"pixel threshold in units of noise rms: "<<Ao<<endl;
 
    gSystem->Exec("date");
    cout<<"core pixels: "<<NET.coherence4(Ao,4.,0.)<<"  "<<endl;
...
/*
    ...
    gSystem->Exec("date");
    cout<<"  pixels: "<<NET.likelihood4('l', true,1.67);
    NET.corrcut(0.1,10,0);                             // remove large glitches
    cout<<"|"<<NET.likelihood4('l',false,1.67);
    NET.corrcut(0.1,0,0);                              // remove large glitches
    for(j=0; j<NET.nLag; j++) { pwc = NET.getwc(j); wc = *pwc; *pwc = wc; }
    NET.setRMS();
    cout<<"|"<<NET.likelihood4('L',false,1.67)<<" \n"; // final likelihood
    NET.setRank(8.);
    NET.corrcut(0.2,0,0);                              // remove large glitches
    gSystem->Exec("date");
